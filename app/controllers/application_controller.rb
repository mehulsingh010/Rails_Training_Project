class ApplicationController < ActionController::Base
  ...
  before_action :authenticate_request, if: :api_request?  # Apply JWT only for API calls
  ...
  # Authenticate users using JWT (applies only to API routes)
    def authenticate_request
      ...
      if decoded.nil?
        ...
        render json: { error: "User not found" }, status: :unauthorized if @current_user.nil?
      end
    end
